...
router.get("/user", async function (req, res) {
	...
	console.log("");
	const identifiers = req.cookies.ms_oid;

	if (!identifiers) return res.status(400).send("No user id");

	const userExists = await userCollection.countDocuments({
		oid: identifiers,
	});
	if (userExists) {
		return userCollection
			...
			.then((userData) => {
				...
				console.log(
					`Retrieved ${userData.courses.length} user courses\n`
				);
				return res.status(200).send(Object.values(userData.courses));
			})
			.catch((err) => {
				console.error(err);
...
router.get("/:identifiers", async function (req, res) {
	...
	if (!identifiers)
		return res
			.status(400)
			.send("No identifiers (college id/course departments) specified.");

	let abbrArray = [];
	if (collegeDepts[identifiers]) {
		// identifiers is a college id
		console.log(
			`Converting college ID "${identifiers}" (${collegeNames[identifiers]}) to department list`
		);
		abbrArray = collegeDepts[identifiers];
	} else if (identifiers.includes(",")) {
		// identifiers is a list of course departments
		abbrArray = identifiers.split(",").map((i) => i.trim());
	} else {
		// identifiers is a user id or single course department
		let userExists;
		if (identifiers.includes("-") && identifiers.length > 30) {
			// likely oid format
			userExists = await userCollection.countDocuments({
				oid: identifiers,
			});
		}
		if (userExists) {
			return userCollection
				...
				.then((userData) => {
					if (!userData || !userData.courses)
						return res
							.status(500)
							.send("Failed to get user courses.");
	...
	if (abbrArray.find((i) => !allDepts.includes(i)))
		return res
			.status(400)
			.send(
				`Identifier "${abbrArray.find(
					(i) => !allDepts.includes(i)
				)}" is not a valid course department.`
			);

	return courseCollection
		.find({ deptID: { $in: abbrArray } }, { projection: { _id: 0 } })
		.toArray()
		.then((docs) => {
			if (!docs || !docs.length)
...
router.get("/search/:query_text", async function (req, res) {
	...
	return courseCollection
		.find({ $or: queryArray }, { projection: { _id: 0 } })
		.toArray()
		.then((docs) => {
			if (!docs) return res.status(500).send("Failed to get courses.");
			console.log(
				`Retrieved ${docs.length} courses from search text: ${query_text}\n`
			);
			return res.status(200).send(docs);
		})
		.catch((err) => {
...
router.get("/:user_id/:course_id", async function (req, res) {
	...
	return userCollection
		.findOne(
			{ oid: user_id, "courses.id": course_id },
			{ projection: { _id: 0, "courses.$": 1 } }
		)
		.then((userData) => {
			...
			return res
				.status(200)
				.send(userData.courses.find((c) => c.id === course_id));
		})
		.catch((err) => {
			console.error(err);
			return res.status(500).send();
...
router.post("/", async (req, res) => {
	const userCollection = req.app.locals.db.collection("users");
	const user_id = req.cookies.ms_oid;
	if (!user_id) return res.status(400).send("No user id specified.");
	const userExists = await userCollection.countDocuments({ oid: user_id });
	if (!userExists) return res.status(404).send("User does not exist.");
	...
	if(!course.id){
		return res.status(500).send("Need valid course id")
	}

	return userCollection
		.updateOne(
			{ oid: user_id },
			{ $set: { [`courses.${course.id}`]: course } }
		)
		.then((dbResult) => {
...
//router.post("/:user_id", async function (req, res) {
	...
	//const validCourseIDs = await courseCollection
	...
	//for (const i in courseArray) {
		...
		//if (!c.id || !validCourseIDs.includes(c.id))
			//return res
				//.status(400)
				//.send(`Course with id "${c.id}" does not exist.`);
		//for (const prop of ["deptID", "courseNumber", "name", "time"])
			//if (!c[prop])
				//return res
					//.status(400)
					//.send(`Course "${c.id}" is missing the field "${prop}".`);
		//if (
			//!c.time.time_start ||
			//!c.time.time_end ||
			//!Array.isArray(c.time.days)
		//)
			//return res
				//.status(400)
				//.send(`Course "${c.id}" has an invalid "time" field.`);
				...
				//for (const j in c[prop])
					//if (!validCourseIDs.includes(c[prop][j]))
						//return res
							//.status(400)
							//.send(
								//`Course "${c[prop][j]}" does not exist (from field "${prop}").`
							//);
		//courses.push({
			//id: c.id,
			//deptID: c.deptID,
	...
	//return userCollection
		//.updateOne({ oid: user_id }, { $set: { courses } })
		//.then((dbResult) => {
			//if (!dbResult || !dbResult.modifiedCount)
				//return res.status(204).send();
			//return res.status(200).send();
