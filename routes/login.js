...
router.get("/", function (req, res) {
	...
	if (error) return res.status(500).send(`${error} - ${error_description}`);
	...
	return axios({
		...
		)}&response_mode=query&scope=openid+profile&code=${code}&client_secret=${encodeURIComponent(
			process.env.CLIENT_SECRET
		)}`,
	})
		.then(async (response) => {
			//console.log(response.data);
			res.cookie("ms_oauth_token", response.data.access_token, {
				maxAge: response.data.expires_in * 1000,
				domain: process.env.HOST,
			});
			res.cookie("ms_oauth_idtoken", response.data.id_token, {
				maxAge: response.data.expires_in * 1000,
				domain: process.env.HOST,
			});
			let oid, email, name;
			try {
				...
				const decodedAT = Buffer.from(atPieces[1], "base64").toString(
					"binary"
				);
			...
			if (!oid) return res.status(500).send("Failed to get identifier.");
			// Check if an account is associated with this OID in the DB.
			return userCollection
				.countDocuments({ oid })
				.then(async (exists) => {
					if (!exists) {
						// If not, create one before redirecting to the dashboard.
						let failedToCreate;
						await userCollection
							.insertOne({ oid, email, name, courses: {} })
							.then((dbResult) => {
